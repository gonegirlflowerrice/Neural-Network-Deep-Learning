...
from tensorflow.keras.layers import Embedding, Dropout, Bidirectional, LSTM, Reshape, Conv2D, MaxPool2D, Flatten, Dense
...
#https://github.com/ManuelVs/NNForTextClassification/blob/master/fen/pzhou_tf.py
#https://github.com/ManuelVs/NNForTextClassification/blob/master/tests/fen/pzhou_tf.py
#https://arxiv.org/pdf/1611.06639v1.pdf
...
def build_classifier_model(embedding_dim, em_drop_rate, sentence_length, lstm_units, blstm_drop_rate, conv_kernel_size, conv_filters, conv_strides, pool_kernel_size, pool_strides, pooling_drop_rate):
    embedding_layer = Embedding(input_dim=len(word_index) + 1,
                                    output_dim=embedding_dim,
                                    weights=[embedding_matrix],
                                    input_length=sentence_length,
                                    trainable=False, name="Embedding_layer")
    em_dropout = Dropout(rate=em_drop_rate, name="em_dropout")

    #merge_mode sum will sum outputs of forward and backward RNN as according to the original implementation. 
    BLSTM_layer = Bidirectional(LSTM(lstm_units, return_sequences=True), merge_mode="sum", name="BLSTM_layer")
    blstm_dropout = Dropout(rate=blstm_drop_rate, name="blstm_dropout")    
    reshape = Reshape(target_shape=[sentence_length, lstm_units, -1], name="blstm_reshape")

    conv_layer = Conv2D(filters=conv_filters, kernel_size=conv_kernel_size, strides=conv_strides, padding="same", use_bias=True, name="Conv2D_layer")

    pooling_layer = MaxPool2D(pool_kernel_size, strides=pool_strides, name="MaxPool2D_layer")
    pooling_dropout = Dropout(rate=pooling_drop_rate, name="pooling_dropout")

    flatten_layer = Flatten(name="Flatten_layer")
    output_layer = Dense(num_classes, activation='softmax', name="Output_Softmax_layer")

    return Sequential([embedding_layer, em_dropout, BLSTM_layer, blstm_dropout, reshape, conv_layer, pooling_layer, pooling_dropout, flatten_layer, output_layer])

def train(name, train_ds, test_ds, embedding_dim, em_drop_rate, sentence_length, lstm_units, blstm_drop_rate, conv_kernel_size, conv_filters, conv_strides, pool_kernel_size, pool_strides, pooling_drop_rate, epochs, batch_size):
    K.clear_session()
    
    ##Construct the classifier model
    model = build_classifier_model(embedding_dim, em_drop_rate, sentence_length, lstm_units, blstm_drop_rate, conv_kernel_size, conv_filters, conv_strides, pool_kernel_size, pool_strides, pooling_drop_rate)
    model.summary()
...
embedding_dim = 300 #acceptable values: 50, 100, 200, 300
...
#train, test = train_test_split(df, test_size=validation_split, random_state=seed)
...
for train_index, test_index in kf.split(X):
    ...
    name = "original_rcnn_fold{}".format(k)
    histories[name] = train(name, train_ds, test_ds, embedding_dim, em_drop_rate, sentence_length, lstm_units, blstm_drop_rate, conv_kernel_size, conv_filters, conv_strides, pool_kernel_size, pool_strides, pooling_drop_rate, epochs, batch_size)
...
results_df.to_csv("rcnn.csv", index=False)
